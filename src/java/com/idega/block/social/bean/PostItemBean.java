...
public class PostItemBean extends ArticleItemBean {
	...
	private void setAccessRights(String resourcePath,IWContext iwc,Collection <Integer> groupIds) throws Exception{
		...
		ace.addPrivilege(new RepositoryPrivilege(Privilege.JCR_ALL));
	    ...
	    try{
		    ...
		    processFolderACL = getRepositoryService().getAuthenticationBusiness().applyPermissionsToRepository(processFolderACL, roleNames);
	...
	protected String generateArticleResourcePath(IWContext iwc) {
		try {
			...
			path.append(getRepositoryService().createUniqueFileName(path.toString(), CoreConstants.ARTICLE_FILENAME_SCOPE)).append(CoreConstants.DOT);
